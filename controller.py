...
class MainController:
    ...
    def launch(self):
        ...
        if len(sys.argv) == 1:
            ...
            taskObj.configObj["detectionThreshold"] = (
                newDetectionThreshold
                if newDetectionThreshold != None
                else taskObj.configObj["detectionThreshold"]
            )
            taskObj.configObj["minTokens"] = (
                newMinsize if newMinsize != None else taskObj.configObj["minTokens"]
            )
        ...
        print("################################################")
        print(
            "#### Starting detection with threshold "
            + str(taskObj.configObj["detectionThreshold"])
            + " ,minsize: "
            + str(taskObj.configObj["minTokens"])
        )
        ...
        os.system(
            "java -Xmx12000m -jar modules/msccd_detector.jar"
            + " "
            + decFolderPath
            + " "
            + tokenBagSourcePath
    ...
    def createTask_new(self):  # Boolean
        ...
        inputObj = InputProject(
            configObj["inputProject"], configObj["languageExtensionName"]
        )
        ...
        tokenizer = os.path.abspath(
            "./tokenizers/"
            + taskObj.configObj["tokenizer"]
            + "/"
            + taskObj.configObj["tokenizer"]
            + "_tokenizer.jar"
        )

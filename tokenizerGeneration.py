...
def parserControllerGeneration(grammarName, path, startRule):
    ...
    str = (
        ...
        + startRule
        + """();pTree = tree;lexicalUnits = tokens.getTokens();}catch (Exception e){ System.out.println("File not found.");return false;}return true;}public ParseTree getPTree(){return pTree;}public List<Token> getLexicalUnits(){return lexicalUnits; }public void reset(){ pTree  = null;lexicalUnits = null; }}"""
    )
...
if __name__ == "__main__":
    ...
    buildingAbspath = (
        rootAbspath + "/modules/msccd_tokenizer/src/main/java/org/nagoya_u/ertl/sa"
    )
    ...
    os.system(
        "bash modules/shells/clearTokenizerBuildingFolder.sh " + parserBuildingPath
    )
    ...
    os.system(
        "bash modules/shells/cpResources.sh "
        + parserSourcePath
        + " "
        + parserBuildingPath
    )
    ...
    parserControllerGeneration(
        configObj["grammarName"], buildingAbspath, configObj["startSymbol"]
    )

    # step5
    os.system(
        "bash modules/shells/package.sh " + rootAbspath + " " + configObj["grammarName"]
    )

    print("###################")
    print(
        "#### Tokenizer for "
        + str(configObj["grammarName"])
        + " is generated in ./tokenizers/"
        + str(configObj["grammarName"])
    )
    print(
        "#### You can use this tokenizer by configuring the field 'tokenizer' of ./config.json to '"
        + str(configObj["grammarName"])
        + "'"
    )
    print("#### Over")

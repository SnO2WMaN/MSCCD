...
def oneDetectExecution(round, resultArr, cloneDetectionController):
    ...
    clonePairsByIndex = cloneDetectionController.cloneDetection(
        bagPool, partialIndex, len(bagPool)
    )
    clonePairsByBag = []
...
class CloneDetectionContraller:
        ...
        for round in range(0, self.MAX_ROUND + 1):
            for pair in resultArr[round]:
                if not self.ifOverlapByFileMap(pair[0], pair[1]):
                    ...
                    clonePairs.append(
                        [
                            [pair[0].fileId, pair[0].bagId],
                            [pair[1].fileId, pair[1].bagId],
                        ]
                    )
        ...
        for queryBlockIndex in range(queryBlockNum):
            ...
            for queryTokenIndex in range(0, subBlockSize):
                ...
                for cBagIndex in partialIndexItem:
                    ...
                    uBound = 1 + min(
                        qBlockSize - queryTokenIndex, cBagSize - cTokenPositionToUpdate
                    )
        ...
        try:
            if cPair[0][0] < cPair[1][0]:
                if self.ifBagOverlap(
                    self.bagCollectionArr[queryPair[0][0]][queryPair[0][1]],
                    self.bagCollectionArr[cPair[0][0]][cPair[0][1]],
                ) and self.ifBagOverlap(
                    self.bagCollectionArr[queryPair[1][0]][queryPair[1][1]],
                    self.bagCollectionArr[cPair[1][0]][cPair[1][1]],
                ):
                    return True
    ...
    def ifBagOverlap(self, bagA, bagB):
        if int(bagA.fileId) != int(bagB.fileId):
            ...
            # else:
            #     return True
            if int(bagA.startLine) >= int(bagB.startLine) and int(bagA.endLine) <= int(
                bagB.endLine
            ):
                return True
            elif int(bagB.startLine) >= int(bagA.startLine) and int(
                bagB.endLine
            ) <= int(bagA.endLine):
                return True
            else:
